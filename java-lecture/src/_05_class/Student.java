package _05_class; // 패키지 = 디렉토리
// 패키지
// 여러 클래스를 가지고 있는 directory
// 클래스를 유일하게 만들어주는 식별자 역할
// - 같은 패키지 안에 같은 클래스가 존재할 수 없도록 구분해줌
// - 서로 다른 패키지에서는 이름이 같은 클래스가 졵재할 수 있음
// 패키지는 특정한 기능을 가지고 있는 클래스를 구분해주는 용도(서로 관련있는 클래스)
// _05_class안에서 또 다른 Student파일을 만들 수 없음

public class Student {
    // 1. 클래스의 필드
    public String name; // 현재의 값은 null
    public int grade;

    // 2. 클래스의 생성자
    public Student(String name, int grade) {
        // 생성자를 통해 값을 초기화
        // this 키워드는 Student 자기 자신을 의미
        this.name = name;
        this.grade = grade;
    }
